...
public class GfxMenuSelectWeapon : MonoBehaviour
{
    ...
    List<GfxPanel> m_panelsEquippedList = new(GfgManagerSaveData.MAX_EQUIPPED_WEAPONS);
    ...
    private void DrawEquippedWeapons(bool aSnapPanelsToDesiredState = false)
    {
        ...
        GfUiTools.CreatePanelList(m_equippedWeaponsParent, panelCreateData, m_panelsEquippedList, Axis.VERTICAL, GfgManagerSaveData.MAX_EQUIPPED_WEAPONS, aSnapPanelsToDesiredState, true, AlignmentHorizontal.MIDDLE, AlignmentVertical.BOTTOM);
    ...
    private void DrawOwnedWeaponsAtPage(int aPage = -1, bool aSnapPanelsToDesiredState = false)
    {
        ...
        if (GfgManagerSaveData.GetActivePlayerSaveData().GetEquippedWeapons().Count == 1)
        {

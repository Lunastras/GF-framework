...
public class GfgClientBehaviour : NetworkBehaviour
{
    [SerializeField]
    ...
    void Start()
    {
        if (HasAuthority)
        {
            GfcManagerServer.Instance.OnPlayerReady += OnPlayerReady;
            GfcManagerServer.Instance.OnPlayerUnready += OnPlayerUnready;
        }

        if (!LoadingScreenManager.CurrentlyLoading)
            GfcManagerServer.SetPlayerIsReady(true, OwnerClientId);
    ...
    protected void OnPlayerReady(ulong aClientId)
    {
        if (aClientId == OwnerClientId && !m_playerIsSpawned.Value)
        {
            ...
            SetPlayerRuntimeGameDataServerRpc(GfgManagerSaveData.GetPlayerRuntimeGameData());
        }
    }
    ...
    {
        if (!GfcManagerServer.HasAuthority) InternalSetPlayerRuntimeGameData(aPlayerRuntimeGameData);
    }

...
public class GfgLoadoutManager : NetworkBehaviour
{
    [SerializeField]
    ...
    [SerializeField]
    protected GfgFiringWeapons m_weaponFiring;
    ...
    void Start()
    {
        if (IsOwner)
        {
            m_hudManager = InvManagerLevel.GetHudManager();
        }
    ...
    */
    ...
    private unsafe bool InternalSetLoadoutWeapon(int aIndexLoadout, int aIndexWeapon, WeaponData aNewWeapon, bool aSwitchToLoadout)
    {
        ...
        bool removeWeapon = !InvManagerWeapons.WeaponExists(aNewWeapon);
        WeaponData currentWeapon = InvManagerWeapons.NullWeapon;
        if (aIndexWeapon < m_loadouts[aIndexLoadout].Count)
            currentWeapon = m_loadouts[aIndexLoadout][aIndexWeapon].Weapon;

        bool currentWeaponNull = !InvManagerWeapons.WeaponExists(currentWeapon);
    ...
    private WeaponGeneric GetWeapon(GameObject aDesiredWeapon, int aDesiredSeed)
    {
        ...
        while (0 <= --index)
        {
            ...
            if (!obj.activeSelf || (wb.GetGfgStatsCharacter() == m_statsCharacter && wb.GetSeed() == aDesiredSeed)) //check if it is inactive or if they have the same character stats
            {
                obj.SetActive(true);
                objectToReturn = wb;
    ...
    private void InternalSetCurrentLoadout(int aIndexLoadout)
    {
        if (null != m_loadouts && 0 < m_loadouts.Count)
        {
            ...
            if (canChangeWeapon)
            {
                ...
                {
                    GameObject desiredWeapon = InvManagerWeapons.GetWeapon(m_loadouts[m_currentLoadOutIndex][i].Weapon);
                    ...
                    weapon.SetStatsCharacter(m_weaponFiring.GetGfgStatsCharacter());
    ...
    */
    ...
    {
        SetLoadoutWeapon(aIndexLoadout, aIndexWeapon, InvManagerWeapons.NullWeapon, aSwitchToLoadout);
    }

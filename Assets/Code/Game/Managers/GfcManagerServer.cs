...
public class GfcManagerServer : NetworkBehaviour
{
    public static GfcManagerServer Instance { get; private set; } = null;
    ...
    public static bool HostReady
    {
        get
        {
            return !NetworkManager.Singleton || (Instance && Instance.m_hostReady.Value);
        }
    }
    ...
    void Start()
    {
        if (HasAuthority)
        {
            for (int i = 0; i < GfgManagerCharacters.GetNumTypes(); ++i)
    ...
    {
        LoadScene(GfgManagerSceneLoader.GetSceneBuildIndexByName(sceneName), loadingMode);
    }

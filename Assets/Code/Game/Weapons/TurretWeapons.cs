...
public class TurretWeapons : NetworkBehaviour
{
    ...
    void Start()
    {
        if (m_statsCharacter == null) m_statsCharacter = GetComponent<GfgStatsCharacter>();
        ...
        for (int i = 0; i < m_turretPhases.Length; ++i)
        {
            ...
            {
                // weapons[j].SetStatsCharacter(m_statsCharacter);
    ...
    private void Update()
    {
        if (GfcManagerServer.HasAuthority || !HasNetworkObject)
        {
    ...
    public void EraseAllBullets(GfgStatsCharacter characterResponsible, float3 centerOfErase, float speedFromCenter, float eraseRadius)
    {
        for (int i = 0; i < m_turretPhases.Length; ++i)
        {
            WeaponGeneric[] systems = m_turretPhases[i].Weapons;

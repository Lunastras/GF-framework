...
public class WeaponChargeLevels : WeaponGeneric
{
    ...
    {
        float deltaTime = Time.deltaTime * GetGfgStatsCharacter().GetDeltaTimeCoef();
        ...
        if (m_isCharging || m_isDischarging)
        {
            ...
            if (m_isDischarging && m_eraseEnemyBullets)
            {
                ...
                if (m_timeUntilNextBulletsErase <= 0)
                {
                    ...
                    GfgManagerCharacters.EraseAllEnemyBullets(GfgStatsCharacter, GfgStatsCharacter.transform.position, m_bombParticleEraseSpeed, m_bombParticleEraseRadius);
                }
            }

...
public class HitBoxSingleBehaviour : HitBoxGeneric
{
    ...
    protected virtual bool HitTarget(GfgStatsCharacter target, float damageMultiplier)
    {
        // Debug.Log("I AM HIT, DESTROY BULLET NOW");
        target.Damage(new(hitBoxValues.damage * damageMultiplier, target.transform.position, Vector3.zero, m_damageType, true, GetGfgStatsCharacter().NetworkObjectId));
        GfcPooling.Destroy(gameObject);
    ...
    public override void CollisionBehaviour(Collider other)
    {
        ...
        GfgStatsCharacter collisionStats = other.GetComponent<GfgStatsCharacter>();
        if (collisionStats != null)
        {
            bool hitSelf = characterStats == collisionStats;
            float damageMultiplier = GfgManagerCharacters.DamageMultiplier(characterStats, collisionStats);

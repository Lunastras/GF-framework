    ...
    void Update()
    {
        ...
        if (Cursor.visible && (GfcCursor.IsMouseOverUI() || m_isDragging) && (m_canMove || m_canChangeSize))
        {
            ...
            else
            {
                ...
                bool canSizeDrag = m_canChangeSize && dragValid && GfcCursor.IsMouseOverUICollision(mousePos, gameObject);
                m_isChangingSize = canSizeDrag && Input.GetMouseButtonDown(0);

                m_lastMousePos = mousePos;
                m_isDragging = m_isChangingSize || (m_canMove && !dragValid && Input.GetMouseButtonDown(0) && GfcCursor.IsMouseOverUICollision(mousePos, gameObject));

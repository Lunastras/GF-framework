...
void InitializeInputData(DitheredV2f input, SurfaceData surfaceData, out InputData inputData)
{
...
void InitializeSurfaceData(ParticleParams particleParams, out SurfaceData surfaceData, float2 screencoords, float distance)
{
    surfaceData = (SurfaceData)0;
    half4 albedo = SampleAlbedoDither(TEXTURE2D_ARGS(_BaseMap, sampler_BaseMap), particleParams, screencoords, distance, _DitherIntensity, _DitherScale); 
...
DitheredV2f vertParticleUnlit(AttributesParticle input)
{
    DitheredV2f output = (DitheredV2f)0;
...
half4 fragParticleUnlit(DitheredV2f input) : SV_Target
{
    UNITY_SETUP_INSTANCE_ID(input);
    ...
    InitializeSurfaceData(particleParams, surfaceData, input.positionSS.xy, dst);
    InputData inputData;

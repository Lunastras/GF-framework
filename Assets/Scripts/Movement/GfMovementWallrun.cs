...
public class GfMovementWallrun : GfMovementSimple
{
    ...
    [SerializeField]
    private float m_wallrunJumpAngle = 35;

    [SerializeField]
    private float m_wallRunDetachForce = 16;
    ...
    [SerializeField]
    private float m_wallrunNormalMinDot = 0.5f;
    ...
    [SerializeField]
    private float m_wallrunJumpCooldownSeconds = 0.1f;
    ...
    protected override void AfterPhysChecks(float deltaTime)
    {
        ...
        if (!m_isWallRunning && !m_touchedParent && null != m_parentTransform.Value && 0 >= m_secondsUntilWallDetach)
        {

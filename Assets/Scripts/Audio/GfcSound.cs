...
public class GfcSound
{
    ...
    {
        GfcManagerAudio.LoadAudioClip(this);
    }

    //todo, make a system that reuses the same audio gameobject in the audio manager for a given GfcSound
    public GfcAudioSource PlaySingleInstance(float aVolume = 1, float aPitch = 1)
    {
        return Play(Vector3.zero, 0, aVolume, aPitch);
    }

    public GfcAudioSource Play(float aDelay = 0, float aVolume = 1, float aPitch = 1)
    {
        return Play(Vector3.zero, aDelay, aVolume, aPitch);
    }
    ...
    {
        return GfcManagerAudio.GetMixerGroup(MixerType);
    }

    public void SetMixerVolume(float aVolume)
    {
        GfcManagerAudio.SetMixerVolume(MixerType, aVolume);
    }

    public void SetMixerVolumeRaw(float aVolume)
    {
        GfcManagerAudio.SetMixerVolumeRaw(MixerType, aVolume);
    }

    public float GetMixerVolumeRaw()
    {
        return GfcManagerAudio.GetMixerVolumeRaw(MixerType);
    }

    public void SetMixerPitch(float aPitch)
    {
        GfcManagerAudio.SetMixerPitch(MixerType, aPitch);
    }

    public float GetMixerPitch()
    {
        return GfcManagerAudio.GetMixerPitch(MixerType);
    }
}